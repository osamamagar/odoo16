...
class HrPayslip(models.Model):
    ...
    #     for contract in contracts.filtered(lambda contract: contract.resource_calendar_id):
    #         hourly_rate = contract.wage / (working_days_per_month * hours_per_day)
    #         day_from = datetime.combine(fields.Date.from_string(date_from), time.min)
    #         day_to = datetime.combine(fields.Date.from_string(date_to), time.max)
    ...
    #         # compute worked days based on attendance records
        ...
        for contract in contracts.filtered(lambda contract: contract.resource_calendar_id):
            ...
            for day, hours, leave in day_leave_intervals:
                ...
                current_leave_struct = leaves.setdefault(holiday.holiday_status_id, {
                    ...
                    'number_of_hours': 0.0,
                    'contract_id': contract.id,
                    'amount': 0.0,  # Adding the amount field to leaves
                })
                current_leave_struct['number_of_hours'] += hours
                work_hours = calendar.get_work_hours_count(
                    tz.localize(datetime.combine(day, time.min)),
            ...
            # Determine total working days in the given period excluding holidays
            total_working_days = sum(
                1 for day in (day_from + timedelta(days=i) for i in range((day_to - day_from).days + 1))
                if calendar.get_work_hours_count(tz.localize(datetime.combine(day, time.min)), tz.localize(datetime.combine(day, time.max))) > 0
            )

            

            # Check if the employee attended all working days
            if total_days >= total_working_days:
                amount = contract.wage  # Full salary if attended all working days
                total_hours = total_working_days * hours_per_day  # Assuming full working hours
            else:
                amount = total_hours * hourly_rate  # Calculate amount based on worked hours
...
# New
    ...
    attendance_amount = fields.Float(string='Attendance Amount', compute='_compute_attendance_amount', store=True)

    attendance_ids = fields.One2many('hr.attendance', 'worked_day_id', string='Attendances')


    # @api.depends('attendance_ids')
    # def _compute_hours_worked(self):
    #     for record in self:
    #         total_hours = sum(attendance.worked_hours for attendance in record.attendance_ids)
    #         record.number_of_hours = total_hours

    # @api.depends('attendance_ids')
    # def _compute_days_worked(self):
    #     for record in self:
    #         total_days = len(record.attendance_ids)  # Assuming each attendance record counts as a day worked
    #         record.number_of_days = total_days

    # @api.depends('number_of_hours', 'contract_id', 'code')
    # def _compute_amount(self):
    #     for record in self:
    #         # Example logic: calculate amount based on number_of_hours, contract_id, code, etc.
    #         if record.code == 'SALARY' and record.contract_id and record.contract_id.resource_calendar_id:
    #             record.amount = (record.number_of_hours / record.contract_id.resource_calendar_id.hours_per_day) * record.contract_id.wage
    #         else:
    #             record.amount = 4
    ...
    # def _compute_amount(self):
    #     for record in self:
    #         if record.contract_id and record.contract_id.resource_calendar_id:
    #             calendar = record.contract_id.resource_calendar_id
    ...
    #             hourly_rate = record.contract_id.wage / (total_working_days * hours_per_day)
                
    #             # Check if the employee attended all working days
    #             if record.number_of_days >= total_working_days:
    #                 record.amount = record.contract_id.wage  # Full salary if attended all working days
    #             else:
    #                 record.amount = record.number_of_hours * hourly_rate  # Calculate amount based on worked hours
    #         else:
    #             record.amount = 0.0  # Default amount if conditions are not met
    ...
    def _compute_amount(self):
        for record in self:
            if record.contract_id and record.contract_id.resource_calendar_id:
                ...
                if record.number_of_days >= total_working_days:
                    record.amount = record.contract_id.wage
                else:
                    record.amount = record.number_of_hours * hourly_rate
            else:
                record.amount = 0.0
    ...
    def _compute_overtime_hours(self):
        for record in self:
            record.overtime_hours = sum(attendance.overtime_hours for attendance in record.attendance_ids)

    @api.depends('attendance_ids')
    def _compute_attendance_hours(self):
        for record in self:
            record.attendance_hours = sum(attendance.worked_hours for attendance in record.attendance_ids)

    @api.depends('overtime_hours')
    def _compute_overtime_amount(self):
        for record in self:
            overtime_rate = record.contract_id.overtime_rate
            record.overtime_amount = record.overtime_hours * overtime_rate

    @api.depends('attendance_hours')
    def _compute_attendance_amount(self):
        for record in self:
            attendance_rate = record.contract_id.attendance_rate
            record.attendance_amount = record.attendance_hours * attendance_rate
